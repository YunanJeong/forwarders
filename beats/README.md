...
- 수동삭제시 서비스 중지/삭제, 설치경로 삭제, 메타데이터경로 삭제 하면된다.
- `윈도우 msi로 설치했을 시, 서비스 제거 후 프로그램 추가/제거로 삭제` 진행
  - 일련의 과정이 자동진행되며, 설치경로 및 메타데이터 경로에서, 변경사항이 있는 파일들은 그대로 남으므로 필요없을시 추가 수동삭제
